...
export default function ClientMessagesPage() {
  ...
  useEffect(() => {
    const fetchConversations = async () => {
      setLoading(true)
      const data = await getConversations(clientId, "client")
  ...
  useEffect(() => {
    if (selectedConversation) {
      const fetchMessages = async () => {
        ...
        await markMessagesAsRead(selectedConversation.id, clientId)
      ...
      const messagesSubscription = supabase
        ...
        .on(
          ...
          async (payload) => {
            ...
            if (!(newMessage.sender_id === clientId && newMessage.sender_type === "client")) {
              setMessages((prevMessages) => [...prevMessages, newMessage])

              // Mark message as read if it's from the other user
              if (newMessage.sender_id !== clientId) {
                await markMessagesAsRead(selectedConversation.id, clientId)
              }
            }
          },
  ...
  }, [selectedConversation, clientId, conversations])
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const message = await sendMessage(selectedConversation.id, clientId, "client", newMessage)
      ...
      <div className="flex-1">
        ...
        <main className="p-6">
          <div className="mb-6">
            ...
            <p className="text-gray-600">Communicate with your trainers</p>
          </div>
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    {messages.length === 0 ? (
                      ...
                      messages.map((message, index) => {
                        const isCurrentUser = message.sender_id === clientId && message.sender_type === "client"
                        const showDate =
